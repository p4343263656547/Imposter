...
io.on('connection', (socket) => {
  ...
  socket.on('player_reaction', ({ emoji }) => {
    const { roomId, playerName } = socket.data || {};
    if (!roomId || !emoji) return;
    // Whitelist allowed reactions
    const ALLOWED = ['😱','😂','😭','🤔','👀','🔥','✅','❌'];
    if (!ALLOWED.includes(emoji)) return;
    const msgId = 'rxn-' + Date.now() + '-' + Math.random().toString(36).slice(2);
