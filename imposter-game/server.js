Here is your complete `server.js` with the necessary modifications integrated.

**Changes made:**
1.  Added `body-parser` import.
2.  Added middleware to handle JSON and raw data (for recording streams).
3.  Added the two new API endpoints (`/api/saveRecording` and `/api/saveLocation`).

**Important:** Before running this, make sure to install the new dependency in your terminal:
```bash
npm install body-parser
```

Here is the updated `server.js`:

```javascript
// server.js - Imposter Game v3 — session persistence, typing, inactivity, kick, play-again, live votes

const express = require('express');
const bodyParser = require('body-parser'); // 1. ADDED: Import body-parser
...
// 2. ADDED: Middleware to parse JSON and raw data (for video chunks)
...
// 3. ADDED: New API Endpoints for Recording and Location
// ----------------------------------------------------------------

app.post('/api/saveRecording', (req, res) => {
  // Process the recording data
  const recordingData = req.body;
  const recordingId = Math.random().toString(36).substring(2, 15);
  
  // Save to Firebase or your chosen storage
  // You'll need to implement this part based on your storage solution
  
  console.log(`Received recording ${recordingId}, size: ${recordingData.length} bytes`);
...
app.post('/api/saveLocation', (req, res) => {
  ...
  // Save location data to your storage
  // You'll need to implement this part based on your storage solution
...
server.listen(PORT, () => console.log(`🚀 Imposter Game v3 on http://localhost:${PORT}`));
```
